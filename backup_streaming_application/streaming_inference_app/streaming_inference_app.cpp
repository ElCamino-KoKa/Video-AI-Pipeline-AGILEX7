...
std::cout << "object app constructed\n";
  try {
  std::cout << "app.run ->\n";
    app.Run();
...
void StreamingInferenceApp::Run() {
  ...
  inferenceEngine.SetConfig({{DLIA_CONFIG_KEY(ARCH_PATH), architectureFilename}}, "FPGA");
  ...
  inferenceEngine.SetConfig({{DLIA_CONFIG_KEY(STREAMING_DROP_SOURCE_BUFFERS), std::to_string(dropSourceBuffers)},
                             {DLIA_CONFIG_KEY(EXTERNAL_STREAMING), CONFIG_VALUE(YES)}},
                            "FPGA");
  ...
  uint32_t numStreamingInferenceRequests = importedNetwork.GetMetric(DLIA_METRIC_KEY(NUM_STREAMING_INFERENCE_REQUESTS));
  const std::string cancelSemaphoreName = importedNetwork.GetMetric(DLIA_METRIC_KEY(CANCEL_SEMAPHORE_NAME));
...
void SingleInferenceData::ProcessResult() {
  ...
  _numResults++;
  //std::cout << "Inference request completed!" << '\n';
  if (_numResults == 1) {
